...
exports.report = function(req, res) {
  req.options.include = [{
    model: OrderItem,
    include: [Design, OrderStatus, Style]
  }, Customer];
  req.options.distinct = true;
  Order.findAndCountAll(req.options).then(function(arrs) {
    res.setHeader('total', arrs.count);
  ...
  Order.findOne({
    ...
    include: [{
      ...
      include: [{
        model: OrderItemMeasurement,
        include: [Measurement]
      }, Style, Material, Design, OrderStatus]
    }, Customer, OrderStatus, User]
  }).then(function(obj) {
    req.order = obj;
    next();
  }).catch(function(err) {
...
exports.create = function(req, res) {
  ...
  if (req.files) {
    for (var index = 0; index < req.files.length; index++) {
      if (req.files[index].fieldname.length > 7) {
        var temp = req.files[index].fieldname.substr(5, 2);
        console.log(req.body.order.OrderItems);
        req.body.order.OrderItems[temp].image = req.files[index].filename;
      } else {
        var temp = req.files[index].fieldname.charAt(5);
        req.body.order.OrderItems[temp].image = req.files[index].filename;
      }
    }
  }
  ...
  if (!req.body.order.id) {
    ...
    _.forEach(req.body.order.OrderItems, function(dt) {
      ...
      _.forEach(dt.OrderItemMeasurements, function(dm) {
        OrderItemMeasurement.update(dm, {
          where: {
            id: dm.id
          }
        });
